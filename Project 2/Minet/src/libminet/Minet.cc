...
int         MinetInit(const MinetModule &mod)
{
    ...
    debug(5) << "Returning from MinetInit() for module " << mod << endl;
    return 0;
}
...
{
    debug(5) << "Calling MinetConnect() for module " << mod << endl;
    ...
    debug(5) << tab << "In MinetConnect(): fifoto=";
    if (fifoto) {
        debug(5) << fifoto << ", fifofrom=";
    }
    else        {
        debug(5) << 0 << ", fifofrom=";
    }
    if (fifofrom)       {
        debug(5) << fifofrom << endl;
    }
    else        {
        debug(5) << 0 << endl;
    }
    ...
    debug(5) << tab << "In MinetConnect(): returned from open()" << endl;
...
MinetHandle MinetAccept(const MinetModule &mod)
{
    ...
    debug(5) << tab << "In MinetAccept(): fifoto=";
    if (fifoto)	{
	debug(5) << fifoto << ", fifofrom=";
    }
    else	{
